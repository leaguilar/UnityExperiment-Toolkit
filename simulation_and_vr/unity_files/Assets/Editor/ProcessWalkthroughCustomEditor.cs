    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Choose raw data directory", GUILayout.Width(buttonWidth)))
        {
            ...
            if (newRawDirectoryName != "" && newRawDirectoryName != processor.rawDataDirectory)
            {
                ...
                processor.outProcessedDataFileName = Path.Combine("Data_VR_Processed", processor.CreateDerivedDataFileName(processor.rawDataDirectory, processor.rawDataFileName, "processed"));
                processor.outSummarizedDataFileName = Path.Combine("Data_VR_Summarized", processor.CreateDerivedDataFileName(processor.rawDataDirectory, processor.rawDataFileName, "summarized"));
            }
        }

        System.IO.DirectoryInfo rawDirInfo = new System.IO.DirectoryInfo(processor.rawDataDirectory);
        string rawParentName = rawDirInfo.Parent != null ? rawDirInfo.Parent.Name : "";
        GUILayout.Label(rawParentName + "/" + rawDirInfo.Name);
